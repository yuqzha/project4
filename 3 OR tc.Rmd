...
```{r}
tc <- c("T_pad_c_2cat", "N_pad_positive", "grade", "ER_negative", "PR_negative", "HER2_positive", "ki67", "histology")
...
cohort <- cohort1 %>%
  ...
  mutate(age_young = case_when(
    ...
    TRUE ~ NA_real_),
    sc_ic = case_when(
     detection_mode == "ScrCa" ~ 0,
     detection_mode == "IntCa" ~ 1,
     TRUE ~ NA_real_)
    )
...
```{r}
# Then run the analysis
cohort_ic <- cohort |> filter(detection_mode == "IntCa")

results <- univariable_logistic(data = cohort_ic, 
                                         group_var = NULL, #"sc_ic", 
                                         outcome_var =  "year1", 
                                         predictor_vars = tc) 
print(results)
```
...
## v3
```{r}
OR_age <- OR_age + theme(legend.position = "none")
OR_pd <- OR_pd + theme(legend.position = "none") 
OR_year_no_y <- OR_year_no_y + theme(legend.position = "none")

fig2v3 <- ((OR_age + ggtitle("A.IntCa vs. ScrCa by age at diagnosis")) + plot_spacer() +
